...
function EmbeddingExampleCard({ providerId, customAlias }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const res = await apiFetch("/api/v1/embeddings", {
        method: "POST",
        headers,
  ...
  return (
    <Card>
      <h2 className="text-sm font-medium mb-4">Example</h2>
  ...
  return (
    <>
      ...
      {modalOpen && (
        ...
        >
          <div
            className="border border-border rounded-lg shadow-[var(--shadow-elev)] w-full max-w-md mx-4 flex flex-col max-h-[80vh]"
...
function GenericExampleCard({ providerId, kind }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api${apiPathWithQuery}`, {
...
function SttExampleCard({ providerId }) {
  ...
  useEffect(() => {
    ...
    apiFetch("/api/tunnel/status")
      ...
      .catch(() => {});
    const loadCustom = () => {
      apiFetch("/api/models/custom", { cache: "no-store" })
        .then((r) => r.json())
        .then((d) => {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const headers = {};
      if (apiKey) headers["Authorization"] = `Bearer ${apiKey}`;
      const res = await apiFetch("/api/v1/audio/transcriptions", { method: "POST", headers, body: fd });
      setLatency(Date.now() - start);
