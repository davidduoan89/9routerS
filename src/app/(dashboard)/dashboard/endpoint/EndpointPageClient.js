...
export default function APIPageClient({ machineId }) {
  ...
  const syncTunnelStatus = async () => {
    try {
      const statusRes = await apiFetch("/api/tunnel/status", { cache: "no-store" });
  ...
  const loadSettings = async () => {
    ...
    try {
      const [settingsRes, statusRes] = await Promise.all([
        ...
        apiFetch("/api/tunnel/status", { cache: "no-store" })
      ]);
      if (settingsRes.ok) {
  ...
  const pingTunnelHealth = async (...urls) => {
    ...
    while (Date.now() - start < TUNNEL_PING_MAX_MS) {
      ...
      if ((Date.now() - start) % 10000 < TUNNEL_PING_INTERVAL_MS) {
        try {
          const statusRes = await apiFetch("/api/tunnel/status");
          if (statusRes.ok) {
  ...
  const handleEnableTunnel = async () => {
    ...
    try {
      const res = await apiFetch("/api/tunnel/enable", { method: "POST" });
      polling = false;
  ...
  const handleDisableTunnel = async () => {
    ...
    setTunnelStatus(null);
    try {
      const res = await apiFetch("/api/tunnel/disable", { method: "POST" });
      const data = await res.json();
      if (res.ok) {
  ...
  const checkTailscaleInstalled = async () => {
    setTsInstalled(null);
    try {
      const res = await apiFetch("/api/tunnel/tailscale-check");
      if (res.ok) {
        const data = await res.json();
  ...
  const handleInstallTailscale = async () => {
    ...
    setTsInstallLog([]);
    try {
      const res = await apiFetch("/api/tunnel/tailscale-install", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleConnectTailscale = async () => {
    ...
    clearUserAuth();
    try {
      const res = await apiFetch("/api/tunnel/tailscale-enable", { method: "POST" });
      const data = await res.json();
      ...
      if (data.needsLogin && data.authUrl) {
        ...
        for (let i = 0; i < 40; i++) {
          ...
          try {
            const r2 = await apiFetch("/api/tunnel/tailscale-check");
            if (r2.ok) {
              const check = await r2.json();
              if (check.loggedIn) {
                clearUserAuth();
                setTsProgress("Starting funnel...");
                const res2 = await apiFetch("/api/tunnel/tailscale-enable", { method: "POST" });
  ...
  const handleDisableTailscale = async () => {
    ...
    setTsStatus(null);
    try {
      const res = await apiFetch("/api/tunnel/tailscale-disable", { method: "POST" });
      const data = await res.json();
      if (res.ok) {
        setTsEnabled(false);
        setTsUrl("");
