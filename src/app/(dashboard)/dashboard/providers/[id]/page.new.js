...
export default function ProviderDetailPage() {
  ...
  const handleSaveSelectedModels = async () => {
    ...
    try {
      const res = await apiFetch(`/api/providers/${activeConnection.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSwapPriority = async (conn1, conn2) => {
    ...
    try {
      ...
      await Promise.all([
        apiFetch(`/api/providers/${conn1.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ priority: p1 }),
        }),
        apiFetch(`/api/providers/${conn2.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      {isCompatible && providerNode && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-sm font-medium">{isAnthropicCompatible ? "Anthropic Compatible Details" : "OpenAI Compatible Details"}</h2>
