...
export default function ModelsCard({ providerId, kindFilter, providerAliasOverride }) {
  ...
  const handleDeleteAlias = async (alias) => {
    try {
      const res = await apiFetch(`/api/models/alias?alias=${encodeURIComponent(alias)}`, { method: "DELETE" });
      if (res.ok) await fetchData();
  ...
  const handleDeleteCustomModel = async (modelId) => {
    try {
      ...
      const res = await apiFetch(`/api/models/custom?${params}`, { method: "DELETE" });
      if (res.ok) {
        await fetchData();
        window.dispatchEvent(new CustomEvent("customModelChanged"));
      }
    ...
    <>
      <Card>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-medium">Models{kindFilter ? ` — ${kindFilter.toUpperCase()}` : ""}</h2>
        </div>
