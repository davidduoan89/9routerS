...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await apiFetch("/api/cli-tools/antigravity-mitm");
      if (res.ok) {
  ...
  const doAction = async (action, password, forceKillPort443 = false) => {
    ...
    try {
      ...
      if (action === "trust-cert") {
        res = await apiFetch("/api/cli-tools/antigravity-mitm", {
          method: "PATCH",
    ...
    <>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 flex w-full max-w-sm flex-col gap-4 rounded-lg border border-border bg-surface p-5 shadow-[var(--shadow-elev)]">
            <h3 className="text-sm font-medium text-text-main">Sudo Password Required</h3>
      ...
      {port443Conflict && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 flex w-full max-w-md flex-col gap-4 rounded-lg border border-border bg-surface p-5 shadow-[var(--shadow-elev)]">
            <h3 className="text-sm font-medium text-text-main">Port 443 Already In Use</h3>
            <div className="flex items-start gap-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              <span className="material-symbols-outlined text-yellow-500 text-[20px]">warning</span>
