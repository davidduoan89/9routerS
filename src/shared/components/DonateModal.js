...
export default function DonateModal({ isOpen, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ref={modalRef}
        className="relative w-full bg-surface border border-border rounded-lg shadow-[var(--shadow-elev)] fade-in max-w-3xl flex flex-col max-h-[85vh]"
      >
...
function DonateChannelCard({ channel }) {
  ...
  return (
    <div className="flex flex-col items-center p-4 rounded-lg border border-border hover:border-pink-500/30 transition-colors">
      {content}
      {url && (
        <a
          href={url}
