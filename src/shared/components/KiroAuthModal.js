...
export default function KiroAuthModal({ isOpen, onMethodSelect, onClose }) {
  ...
  useEffect(() => {
    ...
    const autoDetect = async () => {
      ...
      try {
        const res = await apiFetch("/api/oauth/kiro/auto-import");
  ...
  const handleImportToken = async () => {
    ...
    try {
      const res = await apiFetch("/api/oauth/kiro/import", {
        method: "POST",
          ...
          <div className="space-y-4">
            ...
            {autoDetecting && (
              <div className="text-center py-6">
                ...
                <h3 className="text-sm font-medium mb-2">Auto-detecting token...</h3>
                <p className="text-sm text-text-muted">
