...
const ProviderTopology = lazy(() => import("@/app/(dashboard)/dashboard/usage/components/ProviderTopology"));
const UsageChart = lazy(() => import("@/app/(dashboard)/dashboard/usage/components/UsageChart"));
...
export default function UsageStats({ period: periodProp, setPeriod: setPeriodProp, hidePeriodSelector = false } = {}) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/usage/stats?period=${period}`)
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
  ...
  useEffect(() => {
    const es = new EventSource(`${getApiBase()}/api/usage/stream`);
    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      {loading ? spinner : (
        <div className="grid min-w-0 grid-cols-1 items-stretch gap-2 lg:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
          <Suspense fallback={<div className="h-[300px] animate-pulse rounded-lg bg-surface-2" />}>
            <ProviderTopology
              providers={providers}
      ...
      {loading ? spinner : <Suspense fallback={<div className="h-[300px] animate-pulse rounded-lg bg-surface-2" />}><UsageChart period={period} /></Suspense>}
