...
function isPublicLlmApi(pathname) {
  // Fast prefix check before iterating
  if (pathname[0] !== "/" || pathname[1] !== "v" && pathname[1] !== "a") return false;
...
// Pre-build a Set for O(1) exact matches
const PUBLIC_API_PATHS_SET = new Set(PUBLIC_API_PATHS);

function isPublicApi(pathname) {
  if (isPublicLlmApi(pathname)) return true;
  if (PUBLIC_API_PATHS_SET.has(pathname)) return true;
