...
// In-memory cache for provider connections (hot path during credential selection)
const _connCache = new Map();
let _connCacheTs = 0;
const CONN_CACHE_TTL_MS = 3000; // 3s TTL

function invalidateConnCache() {
  _connCache.clear();
  _connCacheTs = 0;
}
...
export async function getProviderConnections(filter = {}) {
  const now = Date.now();
  if ((now - _connCacheTs) > CONN_CACHE_TTL_MS) {
    _connCache.clear();
    _connCacheTs = now;
  }
  const cacheKey = `${filter.provider || "*"}:${filter.isActive ?? "*"}`;
  if (_connCache.has(cacheKey)) return _connCache.get(cacheKey);
